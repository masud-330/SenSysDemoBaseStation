...
public class Meit
{
  ...
  public IntervalTree buildIntervalTree(int st, int ed,
                                        Vector<Short> listOfPoints,
                                        IntervalTree root)
  {
    if (st == ed)
    {
      IntervalTree leaf_node = new IntervalTree(listOfPoints.get(st).doubleValue(),
                                                root);
      leaf_node.window = new Window(listOfPoints.get(st), // l
                                    listOfPoints.get(st), // r
                                    (short)-5, // h
                                    (short)0); // score
      return leaf_node;
    }
    ...
    IntervalTree new_node = new IntervalTree((listOfPoints.get(mid) +
                                              listOfPoints.get(mid+1))/2,
                                             root);
...
/*  public Vector<Object> readInput(File inputFile, Area area, Area rectangle)
  ...
  public Hashtable<Short, Window> maxEnclosing(Vector<Rectangle> aListOfRectangles,
                             Area coverage, IntervalTree root)
  {
    ...
    // rectangle.)
    int botIndex = 0;
    while(topIndex < aListOfRectangles.size() || botIndex<aListOfRectangles.size())
    {
      // bottom index is always smaller than top index because we
      // process the bottom of a rectangle before we process the top
      // of a rectangle
      if(topIndex < aListOfRectangles.size() && aListOfRectangles.get(topIndex).y1 <= aListOfRectangles.get(botIndex).y2)
      {
        ...
        short curh=aListOfRectangles.get(topIndex).y1;
  ...
  public void writeOutput(String file_name, Area area, Area coverage,
                          Vector<Object> listOfObjects, Window optimal_window)
  {
